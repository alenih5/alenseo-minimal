...
<div class="wrap alenseo-dashboard">
    ...
    <div class="alenseo-charts-section">
        <div class="alenseo-charts-row">
            <div class="alenseo-chart-box">
                <h3><?php _e('SEO-Score Trend', 'alenseo'); ?></h3>
                <div class="alenseo-chart-container">
                    <canvas id="seoTrendChart"></canvas>
                </div>
                <?php
                // Trend-Daten für JavaScript
                $chart_data = $alenseo_dashboard->get_chart_data();
                if (!empty($chart_data['time_series'])) {
                    foreach ($chart_data['time_series'] as $date => $score) {
                        echo '<div class="trend-data" data-date="' . esc_attr($date) . '" data-score="' . esc_attr($score) . '" style="display:none;"></div>';
                    }
                }
                ?>
            </div>
            <div class="alenseo-chart-box">
                ...
                <input type="hidden" id="improve-count" value="<?php echo esc_attr($overview_data['needs_improvement_count']); ?>">
                <input type="hidden" id="no-keywords-count" value="<?php echo esc_attr($overview_data['no_keyword_count']); ?>">
            </div>
        </div>
    </div>
